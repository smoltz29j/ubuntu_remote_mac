...
class Session:
    ...
    FreeRDP 組み込みの +auto-reconnect で回復できず異常終了(exit code != 0)した
    場合に、最大 RETRY_MAX 回・RETRY_INTERVAL 秒間隔で再起動する(Windows 版の
    RdpSessionView と同じ方針)。ユーザーがウィンドウを閉じた場合(exit code 0)や
    「切断」ボタン経由の終了ではリトライしない。
    """
...
class MainWindow:
    def __init__(self, root: tk.Tk):
        ...
        for label, command in [
            ("接続", self.connect), ("切断", self.disconnect),

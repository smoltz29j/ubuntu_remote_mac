...
- **3390 = xrdp 0.10.6(ソースビルド、GFX/H.264 対応)**。2026-07-05 に apt の 0.9.24 から
  移行済み。設定は **`/usr/local/etc/xrdp/`**(`--enable-strict-locations` でビルドされたため)。
...
**`sudo apt install libpipewire-0.3-modules-xrdp` だけでは音は出ない。** Ubuntu のこの
パッケージ(0.2-2)は `libpipewire-module-xrdp.so` **1ファイルしか入れず**、それをロードする
仕組みを含まない。上流 `neutrinolabs/pipewire-module-xrdp` の配布物にある次の 2 つを
別途配置する必要がある(elwhite にはこれが手動配置されていた):

- `/usr/libexec/pipewire-module-xrdp/load_pw_modules.sh` (755, root:root)
- `/etc/xdg/autostart/pipewire-xrdp.desktop` (644, root:root) — 上を `Exec=` で呼ぶ

配置済みマシンから `scp` で持ってくるのが確実(2026-07-19 に elwhite → glavine でそうした。
md5: `2f6f8605d5641306abdf5e7faa324124` / `71825e9dfab024d3253622dd1db795af`)。
動いていれば `pw-cli -m -d load-module libpipewire-module-xrdp sink.node.latency=2048 ...`
がセッション内で走っている。**autostart はセッション開始時にしか実行されない**ので、
配置後は必ずログアウト→再接続すること。
...
- **検証は RDP を経由せずに行う**。`pty` で `su` を駆動して stdin からパスワードを渡す
  スクリプトを作れば、Keychain の値をそのまま PAM に突き合わせられる(値は ssh の暗号化
  経路と stdin のみを通り `ps` に出ない)。`su - <自分> -c ...` は環境によっては
  パスワードを聞かれないので、**プロンプトが出たかを必ず確認**する。

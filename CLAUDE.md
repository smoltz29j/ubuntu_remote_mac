...
- **アプリ側ガード(2026-08-08 実装)**: `other_rdp_clients()` が接続前(初回・自動再接続とも)に
  ...
  (その旨を app.log に「先客チェック不能」で残す)。
- **蹴られた側の検知(2026-09-06 実装)**: exit=2 でログに `ERRINFO_LOGOFF_BY_USER` が
  あれば `Session._detect_kicker()` が切断直後にもう一度 `other_rdp_clients()` を実行し、
  別 PC がいれば状態を「蹴られた」にして相手 IP をダイアログで知らせる。リモートで自分が
  ログアウトした場合は別 PC がいないので通常の「切断」のまま。**実機では未検証**
  (蹴る側の PC を用意して確認すること)。
...
- **コーデックは `/gfx:AVC444 +rfx -nsc`**。sdl-freerdp は `/gfx` を明示しないと rdpgfx
  ...
  初期解像度は画面サイズ(ポイント)の 92% を `/size:WxH` で明示計算する(既定だと 1024x768 で
  開いて小さすぎる。% 指定は前述のとおり SDL クライアントでは効かない)。
